...
describe('mainHeader parsing test', () => {
  it('m_xyxy.spc', () => {
    const mxyxy = fileHeader(
      new IOBuffer(readFileSync(join(import.meta.dirname, 'data/m_xyxy.spc'))),
    ) as TheNewHeader;
  ...
  it('RAMAN.SPC', () => {
    const raman = fileHeader(
      new IOBuffer(readFileSync(join(import.meta.dirname, 'data/RAMAN.SPC'))),
    ) as TheNewHeader;
  ...
  it('m_ordz', () => {
    const mOrdZ = fileHeader(
      new IOBuffer(readFileSync(join(import.meta.dirname, 'data/m_ordz.spc'))),
    ) as TheOldHeader;
